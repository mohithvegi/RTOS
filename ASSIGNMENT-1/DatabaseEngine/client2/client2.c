...
int main(void){
	...
	if((key1 = ftok("/home/dell/IMT2015046/8th sem/RTOS/A/ASSIGNMENT-1/DatabaseEngine/client2", 19)) == -1){
		perror("ftok");
		exit(1);
	}
    ...
    if((key2 = ftok("/home/dell/IMT2015046/8th sem/RTOS/A/ASSIGNMENT-1/DatabaseEngine/client2", 20)) == -1){
		perror("ftok");
		exit(1);
	}
    ...
    if(msgrcv(msgqid2, &buf2, sizeof(buf2), 4, MSG_NOERROR) == -1){
        perror("msgrcv");
        exit(1);
    }
