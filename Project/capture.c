...
static void process_image(void *p, int size)
{
    ...
    buffer_stream = SDL_RWFromMem(p, size);
...
static void init_device(void)
{
        ...
        if (-1 == xioctl(fd, VIDIOC_QUERYCAP, &cap)) { // querry about device capabilities
                if (EINVAL == errno) {
        ...
        if (force_format) {
                fmt.fmt.pix.width       = 160;//640
                fmt.fmt.pix.height      = 120;//480
